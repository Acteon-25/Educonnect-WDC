...
const notificationService = {
	...
	connect: (userId, onNotificationReceived) => {
		const socket = new SockJS('great-emili-educonnect-56088bf2.koyeb.app/ws');
	...
	getNotifications: async (userId, token) => {
		const response = await fetch(`great-emili-educonnect-56088bf2.koyeb.app/educ/notificaciones/${userId}`, {
			headers: {
	...
	markAsRead: async (userId, notificationId, token) => {
		const response = await fetch(`great-emili-educonnect-56088bf2.koyeb.app/educ/notificaciones/${userId}/markAsRead/${notificationId}`, {
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${token}`
			}
	...
	markAllAsRead: async (userId, token) => {
		const response = await fetch(`great-emili-educonnect-56088bf2.koyeb.app/educ/notificaciones/${userId}/markAllAsRead`, {
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${token}`
			}
